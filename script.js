...
function carregarUsuarios() {
    ...
    // Certifique-se de que a variável 'usuarios' está definida e é um array
    if (typeof usuarios === 'undefined' || !Array.isArray(usuarios) || usuarios.length === 0) {
        ...
        // Definir um centro e zoom padrão se não houver dados
    ...
    // Limpa os marcadores existentes antes de recarregar
    marcadores.clearLayers();
    todosMarcadoresUsuario = []; // Limpa o array de referência
    ...
    usuarios.forEach((usuario, index) => {
        if (usuario.latitude && usuario.longitude) {
            ...
            const marcador = L.marker([usuario.latitude, usuario.longitude])
                ...
                `);
            todosMarcadoresUsuario.push(marcador); // Adiciona ao array de referência
        } else {
    ...
    // Adiciona todos os marcadores carregados ao grupo de clustering
    marcadores.addLayers(todosMarcadoresUsuario);
    mapa.addLayer(marcadores);

    // Ajustar zoom para mostrar todos os marcadores de usuário
    if (todosMarcadoresUsuario.length > 0) {
        ...
        // Se não houver usuários com coordenadas válidas, centraliza no Brasil
        mapa.setView(BRASIL_CENTER, BRASIL_ZOOM);
...
function configurarSistemaBusca() {
    ...
    input.addEventListener('input', (e) => {
        ...
        if (termo.length >= 3) {
            ...
            limparMarcadoresUsuarioEncontrado(); // Limpa ao apagar o campo
        }
    });
    ...
    input.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault(); // Evita o submit do formulário se houver um
            const termo = input.value.trim();
            if (termo) {
                executarBuscaGeral(termo);
            }
...
async function executarBuscaGeral(termo) {
    ...
    try {
        ...
        // Ajuste aqui: considerar que um nome pode ter números (ex: "Rua 7")
        // O foco é diferenciar nomes de usuários de endereços genéricos que seriam melhor buscados pelo Nominatim
        ...
        if (isCep) {
            ...
            // Se não é CEP nem nome de usuário, tenta buscar como endereço
    ...
    if (resultados.length === 0) {
        ...
        resultados.forEach(resultado => {
            if (resultado.latitude && resultado.longitude) {
                ...
                // Usa POI_ICON para tipo 'poi' (endereços/comércio)
...
function atualizarMarcadoresVisiveis() {
    ...
    // Contabiliza marcadores de usuários visíveis
    if (clusteringAtivo) {
         // Contamos os child markers dentro dos clusters que estão visíveis no mapa
        marcadores.eachLayer(function(layer) {
            // Verifica se o cluster ou marcador individual está dentro dos limites do mapa
            if (bounds.contains(layer.getLatLng())) {
                if (layer instanceof L.MarkerCluster) {
                    // Para clusters, precisamos contar os marcadores que estão visíveis dentro dele
                    // Não é preciso ser super preciso, a contagem do cluster já indica visibilidade
                    // Se o cluster está visível, considera-se que seus filhos "contribuem" para a visibilidade geral
                    visiveis += layer.getChildCount();
                } else {
                    visiveis++; // Marcador individual visível
                }
            }
        });
    } else {
        // Se o clustering está desativado, iteramos sobre cada marcador individual
    ...
    // Contabiliza marcadores de busca de POI visíveis
    ...
    // Contabiliza marcadores de usuário encontrados pela busca (laranja) visíveis
    marcadoresUsuarioEncontrado.forEach(marker => {
...
function alternarClustering() {
    ...
    atualizarEstatistia(); // Atualiza a estatística do botão
}
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                userLocationMarker = L.marker([lat, lon], {
                    icon: L.icon({
                        iconUrl: `${ICON_BASE_URL}marker-icon-2x-blue.png`,
                        shadowUrl: SHADOW_URL,
                        iconSize: [25, 41],
                        iconAnchor: [12, 41],
                        popupAnchor: [1, -34],
...
function obterDirecoesPara(lat, lon) {
    // Abre o Google Maps com direções para o local
...
// NOVO: Adicionado para alternar estilos de mapa
